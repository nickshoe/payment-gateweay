...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      createdAt: payment.createdAt?.toJSON() ?? null,
      updatedAt: payment.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...
      updatedAt: restPayment.updatedAt ? dayjs(restPayment.updatedAt) : undefined,
    };
  }
